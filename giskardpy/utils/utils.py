...
def get_all_classes_in_package(
    package_name: str, parent_class: Optional[Type] = None, silent: bool = False
) -> Dict[str, Type]:
    """
    ...
    for importer, module_name, ispkg in pkgutil.iter_modules(package.__path__):
        try:
            new_classes = get_all_classes_in_module(
                f"{package.__name__}.{module_name}", parent_class
            )
